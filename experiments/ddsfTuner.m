function [u, ul, descriptions] = ddsfTuner(mode, systype, T_sim)
    ...
    vals = struct( ...
            ...
            'constraints', [inf, 0.1, 0.5, 1.5, 2, 10, 100] ...
    );
    ...
    switch mode
        case 'NvsTini'
            ...
            for i=1:nruns
               ...
               for k=1:max_tries
                    try
                        [~, ~, logs] = runDDSF(systype, T_sim, N_i, T_ini_i, constraint_scaler);
        ...
        case 'constraints'
            ...
            for i=1:nruns
                ...
                for k=1:max_tries
                    try
                        [~, ~, logs] = runDDSF(systype, T_sim, N, T_ini, values(i));
                        u_i = logs.u; % m x T_sim
                        ul_i = logs.ul_t; % m x T_sim                
                        u{end} = u_i;
                        ul{end} = ul_i;
