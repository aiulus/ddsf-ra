function [u_d, y_d, u, y] = generate_data(sys, T)
    A = sys.A;
    B = sys.B;
    C = sys.C;
    D = sys.D;
    ...
    for i = 1:T
       ...
       % disp("x_d(:, i) of size: "); disp(size(x_d(:, i)));
       % disp("A of size: "); disp(size(A));
       % disp("x_d(:, i) of size: "); disp(size(x_d(:, i)));
       % disp("B of size: "); disp(size(B));
       % disp("u_d(:, i) of size: "); disp(size(u_d(:, i)));
        x_d(:, i + 1) = A * x_d(:, i) + B * u_d(:, i);
        %disp("y_d(:, i) of size: "); disp(size(y_d(:, i)));
        %disp("C of size: "); disp(size(C));
        %disp("x_d(:, i) of size: "); disp(size(x_d(:, i)));
        %disp("D of size: "); disp(size(D));
        %disp("u_d(:, i) of size: "); disp(size(u_d(:, i)));
